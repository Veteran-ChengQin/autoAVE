...
def inference(video_path, prompt, device, processor, model,
              max_new_tokens=256,
              total_pixels=20480 * 28 * 28,   # <<< 大幅降低
              min_pixels=224*224):    # <<< 较小底线
    messages = [
        {"role": "system", "content": "You are a helpful assistant."},
        {"role": "user", "content": [
            {"type": "text", "text": prompt},
    ...
    # 打印一下缩放后的帧张量尺寸与 token 数，帮助你观测规模
    num_frames, _, H, W = video_inputs[0].shape
    approx_tokens = int(num_frames / 2 * (H // 28) * (W // 28))
    print(f"[DEBUG] frames={num_frames}, resized={H}x{W}, approx_vision_tokens={approx_tokens}")

    # 单样本无需 padding=True；减少不必要的 pad 张量
    inputs = processor(
        text=[text],
        images=image_inputs,
        videos=video_inputs,
    ...
    with torch.inference_mode(), torch.cuda.amp.autocast(dtype=torch.float16):
        output_ids = model.generate(
            **inputs,
            ...
            do_sample=False,
            use_cache=True,          # 一般推理要开；若真的显存顶不住可设 False
            pad_token_id=processor.tokenizer.eos_token_id
        )

    # 只解码新增部分
    generated_ids = [o[len(i):] for i, o in zip(inputs.input_ids, output_ids)]
    out_text = processor.batch_decode(generated_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--device_id', type=int, default=1, help='CUDA device ID to use')
    parser.add_argument('--data_path', type=str, default="/data/veteran/project/dataE/VideoAVE/Dataset/test_data/clothing_test.csv")
    parser.add_argument('--model_path', type=str, default="/data/share/qwen/Qwen2.5-VL-7B-Instruct", help='Model path')
